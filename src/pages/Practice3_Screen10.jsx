...
const contextNotes = [
  {
    ...
    text: 'Для Лаб 2.5 используйте `size` из `sns.load_dataset("tips")` — это тот же признак, который уже разбирался в Практике 1 и Практике 2.',
  },
  {
    title: 'Выбор модели',
    text: 'В этой лабораторной особенно важно объяснить природу процесса: фиксированное число испытаний ведет к биномиальной модели, поток событий — к Пуассону.',
  },
]
...
# Тот же дискретный признак, что в Практике 1 (экран 10)
# и в Практике 2 (экран 8)
  ...
  return (
    <article className="space-y-6">
      <CourseHeader
        ...
        subtitle="Финальная дискретная лабораторная на уже знакомом ресторанном датасете."
      />

      <section className="content-block space-y-6">
        <DatasetCard
          title="Конкретный источник данных для Лаб 2.5"
          text="Источник данных здесь тоже уже встроен в курс: используйте `tips` из Seaborn и колонку `size`. Это продолжает логику Практики 1 и Практики 2 без смены среды и контекста."
          code={datasetCode}
        ...
        <AlertBox title="Как аккуратно писать про критерий Колмогорова">
          <p>
            В учебной задаче критерий Колмогорова можно использовать как требуемый по условию
            инструмент. Но сильный отчет честно отмечает, что классическая версия теста в первую
            очередь ориентирована на непрерывные распределения.
          </p>
        </AlertBox>
      </section>
