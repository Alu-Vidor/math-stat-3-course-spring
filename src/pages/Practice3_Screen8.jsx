...
const contextNotes = [
  {
    ...
    text: 'В лабораторной работе 2.3 используется датасет `California Housing`. Анализируемый признак: `HouseAge`.',
  },
  {
    title: 'Параметры для Колмогорова',
    text: 'В критерии Колмогорова для признака `HouseAge` следует использовать параметры нормальной модели $\\mu = 28.6395$ и $\\sigma = 12.5856$.',
  },
]
...
const taskItems = [
  ...
  },
  {
    ...
    content:
      'Сопоставьте результаты всех критериев и укажите, приводят ли они к одному и тому же выводу. Если результаты расходятся, это необходимо содержательно прокомментировать.',
  },
]
    ...
    <article className="space-y-6">
      ...
      <section className="content-block space-y-6">
        <DatasetCard
          ...
          text="В лабораторной работе 2.3 используется датасет `California Housing`. Для выполнения задания рассматривается признак `HouseAge`."
          code={datasetCode}
        ...
        <AlertBox title="Как трактовать Jarque-Bera">
          <p>
            Критерий Jarque-Bera не заменяет остальные критерии согласия. При рассмотрении
            нормальной модели он выступает как специальный тест на нормальность. Если основная
            гипотеза связана с другой моделью распределения, его следует интерпретировать как
            дополнительный аргумент против нормальной формы распределения.
          </p>
        </AlertBox>
      </section>
